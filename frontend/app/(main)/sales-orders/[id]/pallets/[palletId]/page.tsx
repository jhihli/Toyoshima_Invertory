...
import type { Pallet, Cargo } from '@/interface/IDatatable';
...
// ── Add Cargo modal ─────────────────────────────────────────────────────────────
function AddCargoModal({ open, nextSeq, composeBarcode, onClose, onSubmit }: {
    ...
    <Overlay onClose={onClose}>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--hair-strong)', borderRadius: 14, boxShadow: '0 24px 60px rgba(20,30,20,0.3)', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--hair)' }}>
          <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>Add Cargo</h2>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', padding: '14px 20px', borderTop: '1px solid var(--hair)', background: 'var(--surface-2)' }}>
          ...
          <button onClick={handleSubmit} disabled={saving} style={{ ...BtnPrimary, opacity: saving ? 0.6 : 1, cursor: saving ? 'not-allowed' : 'pointer' }}>
            {saving ? 'Adding…' : `Add ${n} cargo item${n > 1 ? 's' : ''}`}
          </button>
        </div>
      </div>
    </Overlay>
...
// ── Edit Cargo modal ────────────────────────────────────────────────────────────
function EditCargoModal({ cargo, onClose, onSubmit }: {
  cargo: Cargo | null; onClose: () => void; onSubmit: (d: { note: string }) => Promise<void>;
}) {
  const [note, setNote] = useState('');
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');

  useEffect(() => { if (cargo) { setNote(cargo.note || ''); setError(''); } }, [cargo]);
  ...
  return (
    <Overlay onClose={onClose}>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--hair-strong)', borderRadius: 14, boxShadow: '0 24px 60px rgba(20,30,20,0.3)', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--hair)' }}>
          <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>Edit Cargo</h2>
          <button onClick={onClose} style={{ marginLeft: 'auto', width: 28, height: 28, borderRadius: 7, background: 'var(--surface-2)', border: '1px solid var(--hair)', color: 'var(--ink-3)', display: 'grid', placeItems: 'center', cursor: 'pointer' }}><IClose /></button>
        </div>
        <div style={{ padding: '18px 20px' }}>
          {error && <div style={{ padding: '8px 12px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, color: '#b91c1c', fontSize: 12.5, marginBottom: 14 }}>{error}</div>}
          <div style={{ marginBottom: 14 }}>
            ...
            <div className="mono" style={{ padding: '10px 12px', borderRadius: 9, background: 'var(--surface-2)', border: '1px solid var(--hair)', fontSize: 12.5, color: 'var(--ink-3)', fontWeight: 600, wordBreak: 'break-all' }}>{cargo.barcode}</div>
          </div>
          <div>
            <div style={FieldLabel}>Note <span style={{ fontWeight: 500, letterSpacing: 0, textTransform: 'none', fontSize: 11, color: 'var(--ink-4)' }}>optional</span></div>
...
function ConfirmModal({ target, onClose, onConfirm }: {
  target: Cargo | null; onClose: () => void; onConfirm: () => void;
}) {
    ...
    <Overlay onClose={onClose}>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--hair-strong)', borderRadius: 14, boxShadow: '0 24px 60px rgba(20,30,20,0.3)', overflow: 'hidden', maxWidth: 400, margin: '0 auto' }}>
        <div style={{ padding: '24px 24px 12px', textAlign: 'center' }}>
          ...
          <h2 style={{ margin: '0 0 6px', fontSize: 17, fontWeight: 700 }}>Delete cargo?</h2>
          <p style={{ margin: 0, fontSize: 13.5, color: 'var(--ink-3)', lineHeight: 1.5 }}>Cargo <b className="mono">{target.barcode}</b> will be permanently removed.</p>
        </div>
...
export default function CargoPage() {
  ...
  // cargo barcode to highlight (from the cargo search's ?highlight= param); read client-side to
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [soData, cargoData] = await Promise.all([
        api.sos.get(soId),
        api.pallets.cargos.list(pId),
      ]);
  ...
  // Next C-number: max trailing -C{n} across existing cargo barcodes, + 1 (matches the backend).
  ...
  const handleAdd = async (d: { count: number; note: string }) => {
    const created = await api.pallets.cargos.create(pId, d);
    const n = created?.length ?? d.count;
    await load();
    showToast(`Added ${n} cargo item${n > 1 ? 's' : ''}`);
  };
  const handleEdit = async (d: { note: string }) => {
    if (!editTarget) return;
    await api.pallets.cargos.update(pId, editTarget.id, d);
    await load(); showToast('Cargo saved');
  };
  const handleDelete = async () => {
    if (!deleteTarget) return;
    await api.pallets.cargos.delete(pId, deleteTarget.id);
    await load(); setDeleteTarget(null); showToast('Cargo deleted', 'err');
  };

  const toggleSel = (id: number) => setSelected(s => {
    const n = new Set(s); n.has(id) ? n.delete(id) : n.add(id); return n;
  });
  const allSelected = cargos.length > 0 && cargos.every(c => selected.has(c.id));
  const toggleAll = () => setSelected(allSelected ? new Set() : new Set(cargos.map(c => c.id)));

  const handlePrint = () => {
    const chosen = cargos.filter(c => selected.has(c.id));
  ...
  return (
    <div style={{ padding: isMobile ? '12px 12px 40px' : '22px 28px 40px' }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--hair)', borderRadius: 12, overflow: 'hidden' }}>
        ...
        {/* Card header */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '12px 16px', borderBottom: cargos.length ? '1px solid var(--hair)' : 'none' }}>
          <h2 style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>Cargo Items</h2>
          <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 99, background: 'var(--accent-light)', color: 'var(--accent-2)' }}>{cargos.length}</span>
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
            {selected.size > 0 && (
              <button style={BtnGhost} onClick={handlePrint}><IPrint /> Print ({selected.size})</button>
            )}
            <button style={BtnPrimary} onClick={() => setAddOpen(true)}><IPlus /> Add Cargo</button>
          </div>
        </div>

        {cargos.length === 0 ? null : isMobile ? (
          /* Mobile: card list */
          <div>
            {cargos.map(c => {
      ...
      {/* Modals */}
      <AddCargoModal open={addOpen} nextSeq={nextSeq} composeBarcode={composeBarcode}
        onClose={() => setAddOpen(false)} onSubmit={handleAdd} />
      <EditCargoModal cargo={editTarget} onClose={() => setEditTarget(null)} onSubmit={handleEdit} />
