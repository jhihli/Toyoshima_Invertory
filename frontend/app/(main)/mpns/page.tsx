...
function SoFilter({ selected, onSelect, fullWidth }: {
  ...
  useEffect(() => {
    const onDoc = (e: MouseEvent) => {
      if (boxRef.current && !boxRef.current.contains(e.target as Node)) { setOpen(false); setFocused(false); }
    };
  ...
  const choose = (so: SO) => {
    onSelect({ id: so.id, so_number: so.so_number });
    setOpen(false); setFocused(false); setQuery('');
  };

  const onKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'ArrowDown') { e.preventDefault(); setOpen(true); setHi(p => Math.min(p + 1, options.length - 1)); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); setHi(p => Math.max(p - 1, 0)); }
    else if (e.key === 'Enter') { if (open && options[hi]) { e.preventDefault(); choose(options[hi]); } }
  ...
  if (selected) {
    return (
      <span style={{
        ...
        background: '#ddeef8', border: '1px solid #afd2ea', borderRadius: 6,
        padding: '0 4px 0 9px', fontSize: 12.5, color: '#1a5f8b',
        whiteSpace: 'nowrap', height: 32, boxSizing: 'border-box',
      }}>
        <span style={{ display: 'inline-flex', color: '#4a90b8' }}><FunnelIcon size={11} /></span>
        <span className="mono" style={{ fontWeight: 600, letterSpacing: '0.01em' }}>{selected.so_number}</span>
        <button
          ...
          style={{
            ...
            cursor: 'pointer', color: '#5a9cbf', padding: 0, fontSize: 16, lineHeight: 1,
          }}
          onMouseEnter={e => { e.currentTarget.style.background = 'rgba(26,95,139,0.10)'; }}
  ...
  return (
    <div ref={boxRef} style={{ position: 'relative', width: fullWidth ? '100%' : 210 }}>
      {/* Trigger */}
      <div
        onClick={() => { setOpen(true); inputRef.current?.focus(); }}
        style={{
          display: 'flex', alignItems: 'center', gap: 7, height: 32, padding: '0 8px 0 10px',
          boxSizing: 'border-box', background: 'var(--surface)', cursor: 'text',
      ...
      >
        <span style={{ display: 'inline-flex', color: focused ? 'var(--accent)' : 'var(--ink-4)', flexShrink: 0 }}>
          <FunnelIcon />
        </span>
        <input
          ref={inputRef}
          ...
          style={{
            ...
            fontSize: 13, fontFamily: 'inherit', color: 'var(--ink)', padding: 0, height: '100%',
          }}
        />
        <span style={{ display: 'inline-flex', color: 'var(--ink-4)', flexShrink: 0 }}>
      ...
      {open && (
        <div style={{
          position: 'absolute', zIndex: 30, top: 'calc(100% + 5px)', left: 0,
          width: fullWidth ? '100%' : 320, maxWidth: '90vw',
          background: 'var(--surface)', border: '1px solid var(--hair)', borderRadius: 8,
          boxShadow: '0 10px 28px rgba(15,30,50,0.14)', overflow: 'hidden',
        }}>
          <div style={{
            padding: '7px 12px', fontSize: 10.5, letterSpacing: '0.07em', textTransform: 'uppercase',
            color: 'var(--ink-4)', borderBottom: '1px solid var(--hair)', background: 'var(--surface-2)',
          }}>
            Scope board counts to one SO
          </div>
          <div style={{ maxHeight: 280, overflowY: 'auto', padding: '4px 0' }}>
            {loading ? (
              <div style={{ padding: '14px 12px', fontSize: 12.5, color: 'var(--ink-4)', textAlign: 'center' }}>Searching…</div>
            ) : options.length === 0 ? (
              <div style={{ padding: '14px 12px', fontSize: 12.5, color: 'var(--ink-4)', textAlign: 'center' }}>
                {query.trim() ? 'No matching SOs' : 'No SOs found'}
              </div>
            ) : (
              options.map((so, i) => (
                <button
                  ...
                  style={{
                    ...
                    background: i === hi ? 'var(--accent-light)' : 'transparent',
                  }}
                >
                  <span className="mono" style={{
                    fontSize: 12.5, color: 'var(--ink)', fontWeight: 500, whiteSpace: 'nowrap', flexShrink: 0,
                  }}>{so.so_number}</span>
                  <span style={{
                    flex: 1, minWidth: 0, textAlign: 'right', fontSize: 11.5, color: 'var(--ink-4)',
