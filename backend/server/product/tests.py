...
from product.models import Vendor, SO, Pallet, Box
...
class NextIndexTests(TestCase):
    def test_empty_pallet_starts_at_one(self):
        pallet = make_pallet()
        self.assertEqual(Box.next_index(pallet), 1)
    ...
    def test_counts_web_created_barcodes(self):
        ...
        self.assertEqual(Box.next_index(pallet), 3)
    ...
    def test_device_barcode_does_not_poison_next_index(self):
        ...
        self.assertEqual(Box.next_index(pallet), 2)
...
class BulkBoxCreateTests(TestCase):
    ...
    def test_stores_barcode_verbatim(self):
        ...
        saved = Box.objects.get(pallet=pallet)
    ...
    def test_skips_duplicates_idempotently(self):
        ...
        self.assertEqual(Box.objects.filter(pallet=pallet).count(), 1)
    ...
    def test_non_string_barcode_rejected(self):
        pallet = make_pallet()
        resp = self.client.post(
            self.url(pallet), data=json.dumps({'boxes': [{'barcode': 123}]}),
            content_type='application/json', HTTP_X_API_KEY='test-key',
        )
        self.assertEqual(resp.status_code, 400)
        self.assertFalse(resp.json()['success'])
...
class ScannerBoxListTests(TestCase):
    ...
    def test_lists_boxes(self):
        pallet = make_pallet()
        Box.objects.create(pallet=pallet, barcode='SO112750-hdh77-1-A20260810022801', note='n')
        Box.objects.create(pallet=pallet, barcode='SO112750-hdh77-1-B20260810022802')
        resp = self.client.get(self.url(pallet), HTTP_X_API_KEY='test-key')
        self.assertEqual(resp.status_code, 200)
        boxes = resp.json()['data']['boxes']
        self.assertEqual(len(boxes), 2)
        self.assertEqual(boxes[0]['barcode'], 'SO112750-hdh77-1-A20260810022801')
        self.assertEqual(boxes[0]['note'], 'n')
        self.assertIn('created_at', boxes[0])
